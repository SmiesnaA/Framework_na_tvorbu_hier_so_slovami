...
export class SlovaZacinajuceNa extends Game {
  ...
  async check(word: string) {
    var promise = new Promise(function (resolve, reject) {
      ...
      var first = graph.getFirstNodeName();
      if (word.length < 2) {
        resolve("Slovo musí obsahovať aspoň 2 písmená.");
      } else {
        var lastW = word.substring(0, 1);
        if (lastW == first) {
          resolve("OK");
        }
      }
      resolve("Slovo musí začínať na zadané písmeno.");
    });
    return promise;
  }
...
//--------------------POKRACUJ -----------------------------------
var game = new SlovaZacinajuceNa("client/files/slovaZacinajuceNa.json");
